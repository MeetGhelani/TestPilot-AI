...
export default function AuditPanel({ onBusyChange }: AuditPanelProps) {
  ...
  const DeviceComparisonChart = ({ comparisons }: { comparisons: Record<string, AuditResult> }) => {
    const data = {
      labels: ['Desktop', 'Mobile', 'Low-end'],
      datasets: [
        {
          label: 'Total Score',
          data: [
            comparisons.desktop?.totalScore || 0,
            comparisons.mobile?.totalScore || 0,
            comparisons.lowEndMobile?.totalScore || 0
          ],
    ...
    const options = {
      ...
      plugins: {
        legend: { display: false },
        title: { display: true, text: 'Score Comparison across Devices', color: '#fff' },
      },
      scales: {
    ...
    return <div style={{ height: 240, background: 'var(--surface)', padding: 20, borderRadius: 20, border: '1px solid var(--border)' }}><Bar data={data} options={options} /></div>;
  };
                  ...
                  <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', minHeight: 40 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      ...
                      {metricDescriptions[label] && (
                        <span style={{ fontSize: 9, color: 'var(--text3)', lineHeight: 1.2, maxWidth: 180 }}>{metricDescriptions[label]}</span>
                      )}
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        {info && <span style={{ fontSize: 8, padding: '1px 4px', borderRadius: 4, background: info.color + '22', color: info.color, fontWeight: 800, border: `1px solid ${info.color}44` }}>{info.label}</span>}
                        <span style={{ 
                          fontSize: 16, 
                      ...
                      {info && <span style={{ fontSize: 9, color: 'var(--text3)', fontStyle: 'italic' }}>Limit: {info.limit}</span>}
                    </div>
                  </div>
                );
              })}
          ...
          return (
            <div key={id} 
              ...
              style={{
                ...
                flexDirection: 'column',
                gap: 12,
                boxShadow: isActive ? '0 8px 32px rgba(200,240,105,0.15)' : '0 4px 12px rgba(0,0,0,0.1)',
                position: 'relative',
                overflow: 'hidden',
                cursor: 'pointer',
